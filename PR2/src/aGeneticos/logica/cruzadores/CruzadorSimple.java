 ...
 *
 * Clase que se encarga de la reproducción de los individuos mediante cruce simple. Para ello se hace la conversión de la lista de alumnos
...
public class CruzadorSimple extends Cruzador{
	...
	private Cromosoma getNuevoCromosoma(Cromosoma c, int[] aux) {
		...
		int pos = 0, j, k, cont; boolean encontrado, tapado;
		for (int i = 0; i < lista.length; i++){
			...
			// Desde el que acabamos de tapar hasta el final añadimos un tachado al contador
			for (k = pos + 1; k < lista.length; k++){
				lista[k].addTachado();
			}
		}
		Logger.getLogger("CP").info("----------- Cruce Simple --------------");
		Logger.getLogger("CP").info("Lista de Alumnos: " + listaAlumnos.toString());
		c.delete();
		Logger.getLogger("CP").info(listaRes.toString());

		return new Cromosoma(listaRes, ListaAlumnos.getDesequilibrio(listaRes), ListaAlumnos.getIncompatibles(listaRes));
	}
