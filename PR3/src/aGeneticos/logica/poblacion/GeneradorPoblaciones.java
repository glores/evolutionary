...
public class GeneradorPoblaciones {
	...
	public ArrayList<Cromosoma> generar(int tamPoblacion, ListaAlumnos lista, Evaluador evaluador) {
		...
		ArrayList<PosById> cadena;
		for (int i = 0; i < tamPoblacion; i++) {
			cadena = (ArrayList<PosById>) lista.getPosById().clone();
			...
			Cromosoma individuo = new Cromosoma(cadena, ListaAlumnos.getDesequilibrio(cadena), ListaAlumnos.getIncompatibles(cadena));
			Aleatorio.setLongCromosoma(lista.getSize());
